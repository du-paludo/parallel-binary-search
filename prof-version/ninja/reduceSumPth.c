...
void *reducePartialSum(void *ptr)
{
    ...
    #define MY_DOUBLE_UNROLLED_LOOP 2   
    #define MY_QUADR_UNROLLED_LOOP 2                         
    #define LOOP_TYPE SIMPLE_LOOP    
    //#define LOOP_TYPE MY_DOUBLE_UNROLLED_LOOP        
    //#define LOOP_TYPE MY_QUADR_UNROLLED_LOOP        
    #if LOOP_TYPE == SIMPLE_LOOP
       // work with my chunck
       register float myPartialSum = 0;
       //#pragma unroll 2
       #pragma omp simd reduction(+:myPartialSum)    // com essa opçao ficou 4x mais rapido, inclusive com 4 threads
       for( int i=first; i<=last ; i++ )
            //myPartialSum = plus( myPartialSum, InputVector[i] );
            myPartialSum += InputVector[i];
    #elif LOOP_TYPE == MY_DOUBLE_UNROLLED_LOOP
       register float myPartialSum = 0;
       register float myPartialSum1 = 0;
       for( int i=first; i<=last ; i+=2 ) {
           //myPartialSum = plus( myPartialSum, InputVector[i] );
            myPartialSum += InputVector[i];
            myPartialSum1 += InputVector[i+1];
       }        
       myPartialSum += myPartialSum1;
    #else   
       register float myPartialSum = 0;
       register float myPartialSum1 = 0;
       register float myPartialSum2 = 0;
       register float myPartialSum3 = 0;
       for( int i=first; i<=last ; i+=4 ) {
           //myPartialSum = plus( myPartialSum, InputVector[i] );
            myPartialSum += InputVector[i];
            myPartialSum1 += InputVector[i+1];
            myPartialSum2 += InputVector[i+2];
            myPartialSum3 += InputVector[i+3];
       }        
       myPartialSum += myPartialSum1 + myPartialSum2 + myPartialSum3;
    #endif
